describe('Password-Change Test', () => {
    beforeEach(() => {
        ...
        cy.fillOutLoginForm('a@a.ch', 'aaaaaaaa');
    ...
    afterEach(() => {
        cy.visit('http://localhost/changePassword');
        cy.fillOutChangePasswordForm('passwordChanged', 'aaaaaaaa', 'aaaaaaaa');
    })
    ...
    it('should change the password successfully', () => {
        cy.fillOutChangePasswordForm('aaaaaaaa', 'passwordChanged', 'passwordChanged');
        cy.contains('Passwort erfolgreich geändert').should('exist');
    });

    it('should show error message for incorrect actual password', () => {
        cy.fillOutChangePasswordForm('notTheActualPassword', 'passwordChanged', 'passwordChanged');
    ...
    it('should show error message for new password does not match the confirmation', () => {
        cy.fillOutChangePasswordForm('aaaaaaaa', 'passwordChanged', 'passwordChangedNotMatching');
        cy.contains('Das neue Passwort stimmt nicht mit der Bestätigung überein').should('exist');

        cy.fillOutChangePasswordForm('aaaaaaaa', 'passwordChangedNotMatching', 'passwordChanged');
        cy.contains('Das neue Passwort stimmt nicht mit der Bestätigung überein').should('exist');
    });

    it('should show error message for "new password should not be the same as the actual password"', () => {
        cy.fillOutChangePasswordForm('aaaaaaaa', 'aaaaaaaa', 'aaaaaaaa');
